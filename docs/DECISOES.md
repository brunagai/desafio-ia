...
- **Não há teste automatizado.** A validação da Regra 1 é um `assert` no notebook. Os três casos que mereciam `pytest` são o `CLI-A-1` que acende, o `CLI-A-3` que não acende e um registro sem data.
