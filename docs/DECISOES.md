...
- **Não há teste automatizado neste repositório.** A validação da Regra 1 é o `assert` da célula 18 do notebook, que só existe se a célula for executada. Escrevi uma suíte `pytest` cobrindo a janela do `CLI-A-1` que acende, as fronteiras que não acendem (soma abaixo do corte, R$ 50.000 cravados, R$ 20.000 cravados e menos de 3 operações no dia) e a política de data nula — 6 casos, todos passando —, mas ela ficou **fora da entrega**: a seção 3 do enunciado fixa o conteúdo de `nivel_2/` em `tools.py`, `agente.py` e `confronto.py`, e a instrução é seguir a estrutura exatamente. Optei por cumprir a estrutura e registrar aqui que os dois limiares estritos declarados em "Ambiguidades" continuam garantidos por texto, não por teste.
