...
**Reportar o piso junto com a taxa.** `confronto.py` publica, para cada critério, a taxa do agente e a taxa do melhor agente constante. Pelos dois critérios o agente fica **abaixo** do piso (60% contra 80%; 70% contra 100%). Está no relatório de propósito: taxa de concordância contra regra pobre não sustenta conclusão sozinha, e esconder isso seria pior do que mostrar. O que sustenta é a leitura caso a caso das divergências.

**Parecer fora do contrato não vira risco médio.** O caso real foi `CLI-017`: a LLM devolveu `nivel_risco = alto` com quatro red flags e esqueceu a `justificativa`. A versão antiga descartava a resposta inteira e gravava “médio/indeterminada” — que por acaso batia com o esperado e entrava como *acerto*. Perder o alerta por erro de serialização é incidente, não detalhe. Agora há três estados: `ok`, `reparado` (nível de risco válido, campo de texto ausente marcado explicitamente, mais uma chamada pedindo o formato completo) e `sem_parecer` (nada aproveitável). O nível de risco nunca é remendado pelo pipeline; sem ele, o cliente vai para revisão humana e sai da taxa de concordância.
...
- Sem cache de LLM: dez chamadas no lote gastam cota e ~27 s; em RPM baixo quebraria. Com o retry de formato, um cliente problemático custa duas chamadas.
